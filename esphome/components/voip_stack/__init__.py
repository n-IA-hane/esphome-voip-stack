...
def _validate_signaling_buffers_in_psram(value):
...
DumpDiagnosticsAction = voip_stack_ns.class_("DumpDiagnosticsAction", automation.Action)
...
CONFIG_SCHEMA = cv.All(cv.require_esphome_version(2026, 9, 0), ha_integration.prepare, cv.Schema(
    {
        ...
        # SIP/SDP transaction payloads are retained for RFC retransmission and
        # dialog recovery but never touched by the realtime media path.
        cv.Optional(
            CONF_SIGNALING_BUFFERS_IN_PSRAM, default=False
        ): _validate_signaling_buffers_in_psram,
...
_register_simple_action("voip_stack.dump_diagnostics", DumpDiagnosticsAction)
