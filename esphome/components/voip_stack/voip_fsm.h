...
enum class SipTerminationAction : uint8_t {
  AUTO,
  NONE,
  CANCEL,
  BYE,
...
struct TerminationIntent {
  CallEndReason reason{CallEndReason::LOCAL_HANGUP};
  const char *detail{nullptr};
  SipTerminationAction sip_action{SipTerminationAction::AUTO};
  bool cache_final_response{false};
};

inline SipTerminationAction resolve_sip_termination_action(
    CallState state, SipTerminationAction requested) {
  if (requested != SipTerminationAction::AUTO) return requested;
  switch (state) {
    case CallState::IN_CALL:
      return SipTerminationAction::BYE;
    case CallState::RINGING:
      return SipTerminationAction::FINAL_RESPONSE;
    case CallState::CALLING:
    case CallState::REMOTE_RINGING:
    case CallState::CONNECTING:
      return SipTerminationAction::CANCEL;
    default:
      return SipTerminationAction::NONE;
  }
}
...
template<typename CacheFn, typename SendFn, typename EndFn,
         typename MediaOffFn, typename DisconnectFn>
inline void dispatch_call_termination(const TerminationSnapshot &snapshot,
                                      const TerminationIntent &intent,
                                      CacheFn &&cache_final_response,
                                      SendFn &&send,
                                      EndFn &&end,
  ...
  if (snapshot.transport_connected && snapshot.has_call_id) {
    const SipTerminationAction action =
        resolve_sip_termination_action(snapshot.state, intent.sip_action);
    if (action != SipTerminationAction::NONE) {
      terminal_transaction_pending = send(action);
    }
  }

  end();
  media_off();
  if (!terminal_transaction_pending) disconnect();
}
