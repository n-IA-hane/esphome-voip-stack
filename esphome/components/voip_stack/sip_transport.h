...
class SipTransport : public SipPhoneTransport {
 ...
 protected:
  ...
  bool send_sip_(const SipSignalingString &message, uint32_t ip_v4,
                 uint16_t port);
  bool send_sip_bytes_(const char *data, size_t size, uint32_t ip_v4,
                       uint16_t port);
  bool send_sip_tcp_(const std::string &message);
  bool send_sip_tcp_bytes_(const char *data, size_t size);
  bool send_sip_tcp_record_(const std::string &message, int socket);
  bool send_sip_tcp_record_(const char *data, size_t size, int socket);
  ...
  struct PendingVideoDirectionInvite {
    ...
    std::string branch;
    SipSignalingString offered_sdp;
    uint32_t cseq{0};
  ...
  SipSignalingString last_invite_response_;
  ...
  SipSignalingString confirmed_local_sdp_;
#endif
  ...
  SipSignalingString tcp_tx_pending_;
